...
动态链接和静态链接不同，你可以近似的成是链接部分留到了装载时进行，在使用C语言有一个运行库glibc，它的动态链接库是/lib下的libc.so，程序装载时，操作系统会将动态链接库加载到内存中。
...
关于动态链接库中共享库的函数与数据访问，这里有4种访问类型。

##### 1. 访问同共享库中的函数

这种方法很好解决，因为是同一个共享库，所以直接使用相对地址即可，因为相对地址是不变的，将来不管共享库被如何装载，都能找到对应的函数。
